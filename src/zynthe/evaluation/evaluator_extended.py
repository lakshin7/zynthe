"""
Zynthé EvalX - Enhanced Evaluator with Dual-Model Comparison
...
class DualEvaluator:
    ...
    def evaluate(self, profile_performance: bool = False) -> EvaluationReport:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.dataloader):
                ...
                batch_metrics = compute_extended_metrics(
                    teacher_logits, student_logits, temperature=self.temperature
                )
                ...
                if self.progress_callback and (batch_idx + 1) % self.update_frequency == 0:
                    running_teacher_acc = (
                        teacher_correct / total_samples if total_samples > 0 else 0
                    )
                    running_student_acc = (
                        student_correct / total_samples if total_samples > 0 else 0
                    )
                    ...
                    progress_payload = {
                        ...
                        "progress": (
                            ((batch_idx + 1) / total_batches * 100) if total_batches else None
                        ),
                        ...
                        "teacher_latency_ms": (
                            (teacher_time / total_samples * 1000) if total_samples > 0 else None
                        ),
                        "student_latency_ms": (
                            (student_time / total_samples * 1000) if total_samples > 0 else None
                        ),
                    }
                ...
                if (batch_idx + 1) % 20 == 0:
                    logger.info(
                        f"[DUAL EVAL] Processed {batch_idx + 1}/{len(self.dataloader)} batches"
                    )
        ...
        extended_metrics = compute_extended_metrics(
            teacher_logits_cat, student_logits_cat, temperature=self.temperature
        )
        ...
        if profile_performance:
            ...
            sample_mask = (
                sample_batch["attention_mask"][:8].to(self.device)
        ...
        dist_metrics = {
            **extended_metrics,
            ...
            "batch_confidence_correlation": (
                float(np.mean(confidence_corrs)) if confidence_corrs else 0.0
            ),
            "dei": dei_result["dei"],
            "cas": cas_result["cas"],
        }
        ...
        logger.info(
            f"  CAS: {cas_result['cas']:.4f} | Speedup: {perf_comparison.get('speedup', 0):.2f}x"
        )
        return report
...
class CurriculumEvaluator:
    ...
    def evaluate_by_difficulty(self, dataloader_dict: Dict[str, Any]) -> Dict:
        ...
        for difficulty, dataloader in dataloader_dict.items():
            ...
            results[difficulty] = {"accuracy": accuracy, "metrics": metrics, "num_samples": total}
