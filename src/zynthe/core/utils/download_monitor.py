...
class DownloadProgressMonitor:
    ...
    def log_progress(self, progress: float, message: str, force: bool = False):
        ...
        logger.info(
            f"[PROGRESS] stage={stage} progress={progress:.3f} message={message}", flush=True
        )
...
def wrap_model_loading(
    model_loader_func: Callable, model_id: str, *args, role: str = "teacher", **kwargs
):
    """
    ...
    if is_cached:
        ...
        monitor_thread = threading.Thread(
            target=monitor_cache_directory, args=(model_id, role), daemon=True
        )
        ...
        for cls_name, cls in [
            ...
            if cls_name not in _patched_classes:
                ...
                def make_wrapper(original_method):
                    ...
                    def from_pretrained_with_progress(
                        pretrained_model_name_or_path, *args, _monitor_role="model", **kwargs
                    ):
