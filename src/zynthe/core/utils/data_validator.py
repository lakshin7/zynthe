...
class DataLeakageDetector:
    ...
    def check_overlap(
        ...
        if check_prefixes:
            ...
            results["prefix_overlap_pct"] = (
                (len(near_duplicates) / len(val_set) * 100) if val_set else 0
            )
        ...
        if results["has_exact_leakage"]:
            LOG.error(
                f"[FAIL] DATA LEAKAGE DETECTED! {results['exact_overlap_count']} samples "
    ...
    @staticmethod
    def check_class_balance(labels: List[int], dataset_name: str = "dataset") -> Dict[str, Any]:
        """
        ...
        if results["is_severely_imbalanced"]:
            LOG.warning(
                f"  {dataset_name}: SEVERE class imbalance detected! "
                f"Ratio: {imbalance_ratio:.2f}:1"
            )
            LOG.warning("   Consider using class weights or oversampling")
        elif not results["is_balanced"]:
            LOG.warning(
                f"  {dataset_name}: Class imbalance detected. Ratio: {imbalance_ratio:.2f}:1"
            )
...
class OverfitUnderfitDetector:
    ...
    def analyze_training_curves(
        ...
        val_loss_trend = (
            "increasing"
            if len(val_losses) >= 3 and val_losses[-1] > val_losses[-3]
            else "decreasing"
        )
        ...
        both_losses_high = (
            latest_train_loss > high_loss_threshold and latest_val_loss > high_loss_threshold
        )
        ...
        if loss_gap_pct > 20 and val_loss_trend == "increasing":
            ...
            recommendations.append(
                "Validation loss lower than training loss - check for data leakage!"
            )
        ...
        if train_metrics and val_metrics and len(train_metrics) >= 2 and len(val_metrics) >= 2:
            ...
            metric_gap_pct = (
                (metric_gap / latest_train_metric * 100) if latest_train_metric > 0 else 0
            )
    ...
    def get_training_health_summary(
        train_losses: List[float], val_losses: List[float], metrics_history: Dict[str, List[float]]
    ) -> str:
        """
...
class DataValidator:
    ...
    def validate_dataset_split(
        ...
        results: Dict[str, Any] = {"validation_passed": True, "errors": [], "warnings": []}
        ...
        LOG.info(
            f"  Leakage: {'[FAIL] DETECTED' if leakage_results['has_exact_leakage'] else ' None'}"
        )
    ...
    @staticmethod
    def validate_preprocessing_batch(
        batch: List[Dict[str, Any]], max_errors: int = 5
    ) -> Dict[str, Any]:
    ...
    def assert_preprocessing_ok(train_dataset, val_dataset) -> None:
        ...
        LOG.info(
            f"Preprocessing validation passed: train_checked={train_report['total_checked']} val_checked={val_report['total_checked']}"
        )
