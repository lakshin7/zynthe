...
def _apply_template(raw: Dict[str, Any], cfg: Dict[str, Any]) -> str:
    ...
    if raw.get("prompt") is not None and raw.get("completion") is not None:
        ...
        text = f"{instruction_prefix}{prompt}{response_prefix}{completion}" + (
            eos_token if add_eos else ""
        )
        return text
...
def run_advanced_pipeline(
    raw_samples: List[Dict[str, Any]], tokenizer, config: Dict[str, Any], role: str = "train"
) -> List[Dict[str, Any]]:
    ...
    LOG.info(
        "[ADV-PRE] Starting advanced preprocessing pipeline (%s) on %d samples",
        role,
        len(raw_samples),
    )
    ...
    if profile_lengths and processed:
        ...
        LOG.info(
            "[ADV-PRE] Length stats (%s) count=%d mean=%.1f max=%d p95=%d p99=%d >max=%d",
            role,
            len(lengths),
            mean_len,
            max(lengths),
