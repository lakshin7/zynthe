...
class BasePipeline(ABC, nn.Module):
    ...
    def __call__(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, PipelineMetrics]:
        ...
        for key, value in metrics.custom_metrics.items():
            if isinstance(value, (int, float)):
                self._cumulative_metrics.custom_metrics[key] = float(
                    self._cumulative_metrics.custom_metrics.get(key, 0.0)
                ) + float(value)
            else:
    ...
    def get_cumulative_metrics(self) -> Dict[str, Any]:
        ...
        custom_metrics = {
            key: (
                value / self._total_batches_processed if isinstance(value, (int, float)) else value
            )
