...
class PipelineRegistry:
    ...
    def register(self, name: str, aliases: Optional[list] = None) -> Callable:
        """
        ...
        def decorator(cls: Type[BasePipeline]) -> Type[BasePipeline]:
            ...
            if not issubclass(cls, BasePipeline):
                raise TypeError(f"Pipeline {cls.__name__} must extend BasePipeline")
    ...
    def register_class(
        self, name: str, pipeline_class: Type[BasePipeline], aliases: Optional[list] = None
    ) -> None:
        """
        ...
        if not issubclass(pipeline_class, BasePipeline):
            raise TypeError(f"Pipeline {pipeline_class.__name__} must extend BasePipeline")
    ...
    def create(
        ...
        **kwargs,
        ...
        if pipeline_class is None:
            ...
            raise ValueError(
                f"Unknown pipeline type: '{name}'. " f"Available pipelines: {', '.join(available)}"
            )

        # Create instance
        return pipeline_class(
            teacher=teacher, student=student, config=config, device=device, **kwargs
        )
