...
class ModelInspector:
    ...
    def _compute_detection_confidence(self, info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        arch_level = _level(
            info.get("architecture_family") not in {"unknown", None}, arch_count == 0
        )
    ...
    def _derive_model_capabilities(self, model: nn.Module) -> Dict[str, Any]:
        ...
        supports_adapters = any("adapter" in name for name in module_names) or hasattr(
            model, "add_adapter"
        )
        supports_lora = any("lora" in name for name in module_names) or hasattr(
            model, "lora_layers"
        )
    ...
    def _check_compatibility(self) -> Dict[str, Any]:
        ...
        """
        compat = {"is_compatible": True, "warnings": [], "errors": [], "recommendations": []}
        ...
        if ratio > 10:
            ...
            compat["warnings"].append(  # type: ignore[union-attr,attr-defined]
                f"Low compression ratio ({ratio:.1f}x). " f"Limited efficiency gains expected."
            )
    ...
    def _assess_distillation_difficulty(
        self, layer_mapping: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
    ...
    def _derive_preset_alignment(self) -> Dict[str, Any]:
        ...
        return {
            "primary": {"name": candidate, "confidence": confidence, "signals": signals},
                ...
                mappings.append(
                    {
                        ...
                        "student": s_layer,
                        "confidence": (
                            "high" if abs(i / n_student - t_idx / n_teacher) < 0.2 else "medium"
                        ),
                    }
                )
    ...
    def generate_report(self) -> str:
        ...
        lines = ["=" * 70, "MODEL INSPECTION REPORT", "=" * 70, ""]
        ...
        if "layer_mapping" in report:
            ...
            if mapping["mappings"]:
                ...
                for m in mapping["mappings"][:5]:
                    lines.append(
                        f"  {m['student']} ← {m['teacher']} ({m['confidence']} confidence)"
                    )
                lines.append("")
