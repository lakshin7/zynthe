...
class PreflightAnalyzer:
    ...
    def validate_config(self) -> Dict[str, Any]:
        ...
        if mitigation_enabled:
            validation["info"].append(
                "Adaptive overfit mitigation enabled - staged regularization will run before hard stops"
            )
        else:
            validation["warnings"].append(
    ...
    def _optimize_configuration(self) -> Dict[str, Any]:
        ...
        if optimal_batch > 0:
            optimal_batch = (
                2 ** int(math.floor(math.log2(optimal_batch))) if optimal_batch > 1 else 1
            )
    ...
    def _derive_recommended_preset(self) -> Optional[Dict[str, Any]]:
        ...
        model_arch = (
            self.results.get("model", {}).get("teacher", {}).get("architecture_family", "unknown")
        )
    ...
    def _make_decision(self) -> Dict[str, Any]:
        ...
        if not guard_enabled:
            warning = "Overfit guard disabled - enable train.overfit_guard.enabled to automatically pause when validation loss spikes"
        ...
        if mitigation_enabled:
            ...
            mitigation_warning = "Adaptive overfit mitigation disabled - the system will halt without attempting staged regularization"
        ...
        if not teacher_training_enabled:
            teacher_warning = "Teacher fine-tuning disabled - teacher weights will remain generic during distillation"
        ...
        if decision["can_proceed"]:
            decision["recommendations"].append("All checks passed. Ready to start distillation.")
    ...
    def _format_final_report(self, report: Dict[str, Any]) -> str:
        ...
        """
        lines = ["", "=" * 70, "PREFLIGHT ANALYSIS SUMMARY", "=" * 70, ""]
    ...
    def _compute_risk_profile(self) -> Dict[str, Any]:
        ...
        if isinstance(num_samples, int):
            ...
            if num_samples < 500:
                risk_score -= 10
                rationale.append(
                    "Dataset contains fewer than 500 samples; overfitting risk increases."
                )
        ...
        if not guard_enabled:
            risk_score -= 8
            rationale.append(
                "Overfit guard disabled; enable automatic halt to contain overfitting."
            )
        ...
        if not mitigation_enabled:
            risk_score -= 5
            rationale.append(
                "Adaptive overfit mitigation disabled; guard cannot attempt regularization before stopping."
            )
        ...
        if not teacher_training_enabled:
            risk_score -= 6
            rationale.append(
                "Teacher fine-tuning disabled; teacher quality may bottleneck distillation."
            )
    ...
    def _generate_user_playbook(self) -> Dict[str, List[str]]:
        ...
        if data_report.get("warnings"):
            monitor.append(
                "Dataset warnings detected; review imbalance and preprocessing suggestions."
            )
    ...
    def save_report(self, report: Optional[Dict[str, Any]] = None, format: str = "json") -> Path:
        """
        Save report to file.
        ...
        if format == "json":
            ...
            with open(filepath, "w") as f:
                ...
                if risk:
                    f.write(
                        f"Level: {risk.get('level', 'unknown')} (score {risk.get('score', 0)})\n"
                    )
    ...
    def update_config(
        self, config: Optional[Dict] = None, save_path: Optional[str] = None
    ) -> Dict[str, Any]:
        """
