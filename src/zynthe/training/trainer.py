...
class Trainer:
    def __init__(
        ...
        is_base_model = any(
            indicator in teacher_model_name.lower()
            for indicator in [
                "base-uncased",
                "base-cased",
                "base-multilingual",
        ...
        if is_base_model:
            ...
            logger.info(
                "   This may result in poor distillation. Consider using a fine-tuned model."
            )
        ...
        self.scaler = (
            GradScaler(device=scaler_device, enabled=self.use_amp) if self.use_amp else None
        )
        if self.use_amp:
            logger.info(
                "[OPTIMIZATION] Mixed Precision Training (AMP) enabled - expect 2-3x speedup"
            )
        # 2. Gradient Accumulation - simulate larger batch sizes
        self.gradient_accumulation_steps = int(
            train_cfg.get("gradient_accumulation_steps", train_cfg.get("grad_accum_steps", 1))
        )
        ...
        self.optimizer = OptimizerFactory.get_optimizer(
            self.student, self.config["train"], phase="distillation"
        )
        ...
        self.adaptive_opt = AdaptiveOptimizer(self.optimizer, enable_auto_tune=enable_adaptive)
        ...
        if pipeline is not None:
            ...
            if pipeline_cfg and pipeline_cfg.get("type") in ["multi_stage", "multistage", "multi"]:
                ...
                self.pipeline = PipelineBuilder.from_config(
                    self.config, self.teacher, self.student, self.device
                )
                ...
                if (
                    isinstance(distil_cfg.get("kd_hinton"), dict)
                    and "kd_hinton" not in distiller_config
                ):
        ...
        self.overfit_mitigation_state: Dict[str, Any] = {
            ...
            "history": [],
            "last_epoch": None,
            "counts": {"augmentation": 0, "dropout": 0, "weight_decay": 0, "lr": 0},
        }
        ...
        if self.csv_logging:
            try:
                ...
                self._csv_writer = csv.DictWriter(
                    self._csv_file_handle, fieldnames=self._csv_fieldnames
                )
                if init_file:
        ...
        self.teacher_metrics_history: Dict[str, List[float]] = {
            "accuracy": [],
            "f1": [],
            "precision": [],
            "recall": [],
    ...
    def _log_batch(
        ...
        if (
            (batch_idx + 1) % self.batch_log_interval == 0
            or batch_idx == 0
            or (batch_idx + 1) == batches_total
        ):
    ...
    def _init_mitigation_context(self, train_loader) -> None:
        ...
        if dataset is not None and hasattr(dataset, "augmenter"):
            self.overfit_mitigation_state["baseline_augmentation"] = bool(
                getattr(dataset, "augmenter", None)
            )
        else:
            self.overfit_mitigation_state["baseline_augmentation"] = False

    def _apply_overfit_mitigation(
        self, epoch_idx: int, analysis: Dict[str, Any], train_loader
    ) -> List[str]:
    ...
    def _mitigation_enable_augmentation(self, train_loader) -> Optional[str]:
        ...
        if (
            augmenter is None
            or not getattr(augmenter, "config", None)
            or not augmenter.config.enable
        ):
    ...
    def _mitigation_reduce_lr(self) -> Optional[str]:
        ...
        if self.scheduler is not None and hasattr(self.scheduler, "base_lrs"):
            try:
                ...
                logger.debug("Failed to update scheduler base_lrs")
    ...
    def _compute_supervised_fallback_loss(
        self, student_outputs: Any, labels: Optional[torch.Tensor]
    ...
    def train_epoch(self, dataloader):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            try:
                ...
                logger.warning(
                    f"[WARNING] Failed to move batch to device at index {batch_idx}: {e}"
                )
                continue
            ...
            teacher_runtime_kwargs = self._build_forward_runtime_kwargs(
                self._teacher_forward_params
            )
            student_runtime_kwargs = self._build_forward_runtime_kwargs(
                self._student_forward_params
            )
            ...
            amp_dtype = (
                torch.bfloat16 if self.use_amp and self.device.type == "cpu" else torch.float16
            )
            with torch.no_grad():
                    ...
                    with torch.amp.autocast(device_type=self.device.type, dtype=amp_dtype):
                        try:
                            teacher_outputs = self.teacher(
                                **teacher_batch, **teacher_runtime_kwargs
                            )
                        except Exception as e:
                            logger.warning(
                                f"[WARNING] Teacher forward pass failed at batch {batch_idx}: {e}"
                            )
                            continue
                else:
                    try:
            ...
            if self.use_amp and self.device.type != "mps":
                with torch.amp.autocast(device_type=self.device.type, dtype=amp_dtype):
                    try:
                        ...
                        logger.warning(
                            f"[WARNING] Student forward pass failed at batch {batch_idx}: {e}"
                        )
                        continue
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[WARNING] Loss computation failed at batch {batch_idx}: {e}"
                        )
                        fallback_loss = self._compute_supervised_fallback_loss(
                            student_outputs, labels
                        )
                        if fallback_loss is None:
                            logger.warning(
                                f"[WARNING] Skipping batch {batch_idx}: no valid fallback loss"
                            )
                            continue
            ...
            if accumulation_counter >= self.gradient_accumulation_steps:
                ...
                grad_norm = GradientManager.clip_gradients(
                    self.student, max_norm=max_grad_norm, norm_type=2.0
                )
            ...
            try:
                ...
                logger.debug("Failed to append train batch loss")
            num_batches += 1
        ...
        try:
            micro_path = os.path.join(
                self.experiment_dir, f"student_epoch{len(self.train_losses)}_train_micro.png"
            )
            plot_epoch_micro_series(
            ...
            LOG.warning(
                f"Failed to plot student micro-series for epoch {len(self.train_losses)}: {e}"
            )
    ...
    def evaluate(self, dataloader, compute_extended=True) -> EvaluationReport:
        ...
        if len(dataloader) > 0:
            ...
            if "input_ids" not in sample_batch:
                LOG.warning(
                    "Evaluation batch missing 'input_ids'. This might crash the forward pass."
                )
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                if torch.cuda.is_available() and "cuda" in device_str:
                    try:
                        ...
                        LOG.debug(
                            "Unable to read CUDA memory stats during evaluation", exc_info=True
                        )
                labels = batch.get("labels", None)
                feature_payload = self._collect_distiller_features()
                try:
                ...
                try:
                    ...
                    logger.debug("Failed to append validation loss")
                num_batches += 1
                ...
                if labels is not None:
                    ...
                    if student_logits is not None and hasattr(student_logits, "dim"):
                        if (
                            student_logits.dim() == 3
                            and hasattr(labels, "dim")
                            and labels.dim() >= 2
                        ):
                            is_token_task = True
                            ...
                            try:
                                if prob_tensor.dim() >= 2:
                                    confidence_samples.extend(
                                        prob_tensor.max(dim=-1)[0].detach().cpu().tolist()
                                    )
                                else:
                                    confidence_samples.extend(prob_tensor.detach().cpu().tolist())
                            except Exception:
                                logger.debug("Failed to extract confidence samples")
                        ...
                        if not is_token_task:
                            try:
                                ...
                                logger.debug("Failed to update running accuracy")
        ...
        try:
            micro_val_path = os.path.join(
                self.experiment_dir, f"student_epoch{len(self.val_losses)}_eval_micro.png"
            )
            plot_epoch_micro_series(
            ...
            LOG.warning(
                f"Failed to plot student eval micro-series for epoch {len(self.val_losses)}: {e}"
            )
        ...
        if is_token_task and token_total > 0:
            ...
            try:
                computed_metrics = compute_all_metrics(
                    all_preds, all_labels, pred_probs=pred_probs_np
                )
                ...
                calibration_payload = (
                    computed_metrics.get("calibration")
                    if isinstance(computed_metrics.get("calibration"), dict)
                    else None
                )
                diagnostics = self._build_eval_diagnostics(
                    computed_metrics,
            ...
            try:
                ...
                try:
                    extended_metrics = compute_extended_metrics(
                        teacher_logits_cat, student_logits_cat, temperature=temperature
                    )
                except Exception as e:
                ...
                print(
                    f"[EXTENDED] KL: {extended_metrics['kl_divergence']:.4f}, "
        ...
        if latency_ms:
            ...
            if total_time_s > 0 and samples_processed > 0:
                runtime_snapshot["throughput_samples_per_s"] = float(
                    samples_processed / total_time_s
                )
    ...
    def fit(self, train_loader, val_loader):
        ...
        try:
            ...
            if train_samples and val_samples:
                validation_results = DataValidator.validate_dataset_split(
                    train_samples, val_samples, text_key="text", label_key="label"
                )
        ...
        if self.should_train_teacher:
            ...
            logger.info("=" * 70 + "\n")
            logger.info(
                f"[INFO] Training teacher for {self.teacher_epochs} epochs before distillation..."
            )
        ...
        for epoch in range(start_epoch, epochs):
            ...
            val_loss = (
                val_report.loss
                if val_report.loss is not None
                else float(val_report.metrics.get("loss", 0.0))
            )
            ...
            if val_loss < self.best_val_loss:
                ...
                self.no_improve_epochs = 0
                logger.info(
                    f"[INFO] Validation loss improved to {val_loss:.4f}. Saving best model state."
                )
            else:
            ...
            guard_triggered = self._maybe_trigger_overfit_guard(
                epoch + 1, train_loader=train_loader
            )
            ...
            if save_epochs > 0 and (epoch + 1) % save_epochs == 0:
                ...
                ModelSaver.save_training_run(
                    ...
                    best_metric=(
                        val_metrics_dict.get("accuracy")
                        if isinstance(val_metrics_dict, dict)
                        else None
                    ),
        ...
        if len(self.train_losses) >= 2 and len(self.val_losses) >= 2:
            training_health = OverfitUnderfitDetector.get_training_health_summary(
                self.train_losses, self.val_losses, self.metrics_history
            )
            ...
            try:
                ...
                health_analysis = OverfitUnderfitDetector.analyze_training_curves(
                    ...
                    self.metrics_history.get(
                        "accuracy", []
                    ),  # Will be split by train/val in future
        ...
        try:
            ...
            if (
                self.last_preds
                and self.last_labels
                and len(self.last_preds) == len(self.last_labels)
            ):
            ...
            if (
                getattr(self, "teacher_last_preds", None)
                and getattr(self, "teacher_last_labels", None)
                and len(self.teacher_last_preds) == len(self.teacher_last_labels)
            ):
                teacher_cm_dir = os.path.join(self.experiment_dir, "teacher_confusion")
                os.makedirs(teacher_cm_dir, exist_ok=True)
                teacher_metrics = compute_all_metrics(
                    self.teacher_last_preds, self.teacher_last_labels
                )
        ...
        if self.enable_comparison_plot and self.teacher_epoch_losses and self.train_losses:
            try:
                ...
                comparison_path = os.path.join(
                    self.experiment_dir, "teacher_student_comparison.png"
                )
                plot_teacher_student_comparison(
                ...
                if hasattr(self, "final_report") and self.final_report:
                    ...
                    plot_distillation_gap(
                        teacher_metrics={
                            k: v[-1] if v else 0.0 for k, v in self.teacher_metrics_history.items()
                        },
                        student_metrics={
                            k: v[-1] if v else 0.0 for k, v in self.metrics_history.items()
                        },
                        save_path=gap_path,
                    )
            except Exception as e:
    ...
    def _train_teacher(self, train_loader, val_loader):
        ...
        for epoch in range(self.teacher_epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                try:
                    ...
                    logger.debug("Failed to append teacher train batch loss")
                ...
                try:
                    ...
                    if (
                        ...
                        and hasattr(logits, "dim")
                        and logits.dim() >= 2
                    ):
                        preds = torch.argmax(logits, dim=-1)
                        running_train_correct += int((preds == labels).sum().item())
                        running_train_total += int(labels.numel())
                        running_train_acc_series.append(
                            running_train_correct / max(running_train_total, 1)
                        )
                except Exception:
                    logger.debug("Failed to compute teacher training accuracy")
        ...
        try:
            teacher_train_micro = os.path.join(
                self.experiment_dir, f"teacher_epoch{epoch+1}_train_micro.png"
            )
            plot_epoch_micro_series(
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                try:
                    ...
                    logger.debug("Failed to append teacher validation batch loss")
                ...
                if labels is not None:
                    ...
                    try:
                        ...
                        running_val_acc_series.append(
                            running_val_correct / max(running_val_total, 1)
                        )
                    except Exception as e:
                        logger.warning(f"Failed to compute teacher validation accuracy: {e}")
                ...
                for batch in val_loader:
                    ...
                    if labels is not None:
                        ...
                        try:
                            ...
                            logger.debug("Failed to compute teacher validation accuracy")
                    # Detailed eval batch logging for teacher
                    batch_size = 0
                    labels_tensor = batch.get("labels") if isinstance(batch, dict) else None
                    if labels_tensor is not None and hasattr(labels_tensor, "shape"):
            ...
            try:
                teacher_eval_micro = os.path.join(
                    self.experiment_dir, f"teacher_epoch{epoch+1}_eval_micro.png"
                )
                plot_epoch_micro_series(
            ...
            if all_preds and all_labels:
                ...
                if is_token_grid:
                    ...
                    ignore_index = int(
                        self.config.get("distillation", {}).get("ignore_index", -100)
                    )
            ...
            print(
                f"[TEACHER] Epoch {epoch+1}: Train Loss={avg_train_loss:.4f}, "
        ...
        if self._csv_writer and hasattr(self, "_csv_file_handle"):
            try:
                ...
                logger.debug("Failed to flush CSV file handle")
    ...
    def _maybe_trigger_overfit_guard(self, epoch_idx: int, train_loader=None) -> bool:
        ...
        should_react = (
            status in ("overfitting", "mild_overfitting")
        ...
        if should_react:
            ...
            if cfg["mode"] in ("early_stop", "stop", "halt"):
                logger.info(
                    "[OVERFIT-GUARD] Halting student training to prevent further overfitting."
                )
                return True
    ...
    def resume_from_checkpoint(self, checkpoint_dir: str):
        ...
        try:
            ...
            if model_file.exists():
                ...
                self.student = self.student.__class__.from_pretrained(str(ckpt_path)).to(
                    self.device
                )
            else:
            ...
            try:
                ...
                if isinstance(metadata, CheckpointMetadata):
                    ...
                    if isinstance(payload, dict):
                        ...
                        self.resume_global_step = int(
                            max(0, int(payload.get("global_step", 0) or 0))
                        )
