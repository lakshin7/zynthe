"""
Zynthé Toolkit - Advanced Scheduler System
===========================================
...
class SchedulerFactory:
    ...
    def get_scheduler(self, num_training_steps: Optional[int] = None):
        ...
        if warmup_steps > 0 and scheduler_type != "onecycle":  # OneCycle has built-in warmup
            ...
            scheduler = WarmupScheduler(
                scheduler, warmup_steps=warmup_steps, warmup_type=warmup_type
            )
    ...
    def _create_cosine_scheduler(self, num_training_steps: Optional[int]):
        ...
        LOG.info(
            f"Cosine scheduler: total_steps={num_training_steps}, "
            f"warmup_steps={warmup_steps}, effective_steps={effective_steps}, "
            f"eta_min={eta_min}"
        )

        scheduler = CosineAnnealingLR(
            self.optimizer, T_max=effective_steps, eta_min=eta_min  # Use effective steps, not total
        )
        return scheduler
    ...
    def _create_polynomial_scheduler(self, num_training_steps: Optional[int]):
        ...
        scheduler = PolynomialLR(self.optimizer, total_iters=num_training_steps, power=power)
        return scheduler
    ...
    def _create_cyclic_scheduler(self):
        ...
        scheduler = CyclicLR(
            self.optimizer, base_lr=base_lr, max_lr=max_lr, step_size_up=step_size_up, mode=mode
        )
        return scheduler

    def _create_multistage_scheduler(self, num_training_steps: Optional[int]):
        """Create multi-stage scheduler for complex pipelines."""
        stages = self.config.get(
            "stages",
            [{"scheduler": "linear", "steps": 1000}, {"scheduler": "cosine", "steps": 4000}],
        )

        scheduler = MultiStageScheduler(
            self.optimizer, stages=stages, num_training_steps=num_training_steps
        )
        return scheduler
...
class WarmupScheduler:
    ...
    def __init__(self, scheduler, warmup_steps: int, warmup_type: str = "linear"):
        """
        Args:
...
def get_scheduler(
    optimizer: Optimizer, config: Optional[Dict] = None, num_training_steps: Optional[int] = None
):
    """
