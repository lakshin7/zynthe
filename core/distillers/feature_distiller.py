...
class FeatureMetrics:
    ...
    @staticmethod
    def fsp_loss(f_t_pair: Tuple[torch.Tensor, torch.Tensor],
                 f_s_pair: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
    ...
    def align_features(
        ...
        if f_t.shape[1] != f_s.shape[1]:
            adapter = self.create_adapter(
                student_channels=f_s.shape[1],
                teacher_channels=f_t.shape[1],
