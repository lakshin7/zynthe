...
from core.pkg.manifest import ArtifactRecord, MANIFEST_FILENAME, Manifest, compute_sha256
...
class RuntimeOptions:
    ...
    def from_namespace(cls, args: Any, overrides: Optional[Dict[str, Any]] = None) -> "RuntimeOptions":
        return cls(
            ...
            use_teacher_agent=bool(getattr(args, "use_teacher_agent", False)),
        )
...
class UnifiedTrainingRuntime:
    ...
    def run(self, options: RuntimeOptions) -> RuntimeResult:
        ...
        from core.preflight.analyser import validate_config_only, run_preflight_check
        ...
        teacher, student, tokenizer = load_models(
            cfg_manager,
            cfg_manager.device(),
            use_agent=bool(options.use_teacher_agent),
        )
